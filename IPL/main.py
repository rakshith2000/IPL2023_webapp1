...
clr = {'CSK': {'c1':'#fdcd05', 'c2':'#f15c19', 'c3':'#00adef'},
        ...
        'KKR':{'c1':'#3a225d', 'c2':'#64517c', 'c3':'#f2c028'},
        'LSG':{'c1':'#a72056', 'c2':'#f28b00', 'c3':'#ffcc00'},
        'MI':{'c1':'#004ba0', 'c2':'#0077b6', 'c3':'#d1ab3e'},
        'PBKS':{'c1':'#ed1d24', 'c2':'#f2d1a0', 'c3':'#a7a9ac'},
...
def index():
    if db.session.execute('select count() from user').scalar() == 0:
        user = User(email='adminipl2023@gmail.com', \
                    password=generate_password_hash('Admin@ipl2023', method='sha256'), \
                    name='AdminIPL2023')
        db.session.add(user)
        db.session.commit()
    ...
    if db.session.execute('select count() from squad').scalar() == 0:
        df = open('IPL/all teams squad.csv', 'r')
        df = list(csv.reader(df))
        for i in df[1:]:
...
def displayPT():
    ...
    for i in dataPT:
        ...
        dataFR = db.session.execute('select Team_A,Team_B,Result from Fixture where Team_A=="{}" or Team_B=="{}"'.format(i.team_name, i.team_name))
        nm = '--'
        for j in dataFR:
            if j[2] != None:
                continue
